RAGFlow-Datago – Workspace Guide

This document explains every directory and key file in this workspace, how to run KataGo on GPU, where configs and models live, and how the reorganized RAG-augmented research workspace is structured now that the former `datago/` tree has been merged into the repository root.
...
Project layout
  ...
  /scratch2/f003x5w/RAGFlow-Datago
  ├─ katago_repo/                # KataGo source + builds (OpenCL/Eigen) and configs/models
  ├─ src/                        # RAG-enhanced bot implementation (bot, mcts, memory, gating, etc.)
  ├─ scripts/                    # Data/RAG tooling (populate_memory, run_rag_harness, etc.)
  ├─ testing/                    # Match harnesses, tmux helpers, quick tests, and log outputs
  ├─ rag_store/                  # Game analyzer, ANN tools, rag_output shards, and docs
  ├─ docs/                       # Architecture notes, implementation guides, experiment logs
  ├─ data/                       # Memory snapshots, SGF samples, scratch tmp files
  ├─ extra_files/                # Archived Go_env virtualenv and KataGo config bundle
  ├─ notebooks/, tuning/, tests/ # Analysis notebooks, tuning plans, and import smoke tests
  ├─ Go_env/ / venv/             # Python environments (GPU Go_env + general venv)
  └─ logs/, gtp_logs/, rag_test_output.log, etc. for runtime artifacts
