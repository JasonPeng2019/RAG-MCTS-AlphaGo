...
def run_match(
    ...
    # Start KataGo GTP engine (GPU is controlled via config/override)
    logger.info("Starting KataGo GTP engine...")
    cmd = [
        katago_executable,
        'gtp',
        '-model', katago_model,
    ...
    def score_current_game(context: str) -> str:
        """
        Use KataGo's final_score to determine the winner of the current game.
        
        Args:
            context: Message describing why scoring is triggered.
        
        Returns:
            Result string: "DataGo", "KataGo", or "Draw".
        """
        logger.info(context)
        ...
        if success and score_result:
            ...
            if score_result.startswith('B+'):
                ...
                return "DataGo"
            if score_result.startswith('W+'):
                logger.info("KataGo (White) wins by scoring!")
                return "KataGo"
            logger.info("Score returned no winner, recording as Draw.")
            return "Draw"
        ...
        try:
            ...
            while move_number < max_moves:
                ...
                if move == "RESIGN":
                    ...
                    passes += 1
                    if passes >= 2:
                        result = score_current_game("\nBoth players passed. Scoring game...")
